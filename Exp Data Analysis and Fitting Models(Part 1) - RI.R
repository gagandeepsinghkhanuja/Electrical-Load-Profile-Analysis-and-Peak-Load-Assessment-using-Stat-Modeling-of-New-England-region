...
par(mfrow=c(3,2))
for (i in 1:22) {
  hist(ri1[,i], xlab=names(ri1)[i], main=names(ri1)[i], col="blue", prob=T)
  lines(density(ri1[,i]), col="red", lwd=2) #adds density line
}
...
library(corrplot)
library(Hmisc)
r <- cor(ri[,c(3,5:23,25,26)])
res<- rcorr(as.matrix(ri[,c(3,5:23,25,26)]))
...
#we keep DryBulb.avg, WetBulb.min, Dewpoint.max, StPress.max
#and remove date,state,county and year

ri2<-subset(ri1, select = c(-2,-3,-5,-7,-9,-10,-18,-19,-20))
...
plot1<-ggplot(ri, aes(x=Peak.Demand))+ 
...
scplot1<-ggplot(ri, aes(x=DryBulb.avg, y=Peak.Demand)) + 
...
ri3<-subset(ri2, select = c(-6,-7,-8,-9,-12))
mlr2<-lm(Peak.Demand~.,data = ri3)
summary(mlr2)
alias(mlr2)
#Very low R-square value 0.1315 


#Training and Testing------------------------------------------------------------------------------
#Splitting the data into training and testing sets
sample_data<-sample(length(ri1[,1]),size=0.85*(length(ri1[,1])),replace=FALSE,set.seed(9))
train.df<-ri1[sample_data,]
test.df<-ri1[-sample_data,]
...
mlr4<-lm(Peak.Demand ~ WetBulb.min + RelHumi.avg + DryBulb.min + 
           StPress.min + DewPoint.min + DewPoint.avg + DryBulb.avg + 
           WetBulb.avg + WindSpeed.avg + RelHumi.max + StPress.max + 
           DewPoint.max + WetBulb.max + DryBulb.max + RelHumi.min + 
           WindSpeed.min + Population + PopDensity, data = train.df)
summary(mlr4)
#R-square improves to 0.1852
...
rmse.mlr
#High RMSE of 166 achieved. Therefore, further investigation required
...
library('MASS')
#Boxcox for mlr4
BC<-boxcox(Peak.Demand ~WetBulb.min + RelHumi.avg + DryBulb.min + 
             StPress.min + DewPoint.min + DewPoint.avg + DryBulb.avg + 
             WetBulb.avg + WindSpeed.avg + RelHumi.max + StPress.max + 
             DewPoint.max + WetBulb.max + DryBulb.max + RelHumi.min + 
             WindSpeed.min + Population + PopDensity, data = train.df,lambda = seq(-4,4))
lamdamax<- BC$x[which.max(BC$y)]
lamdamax
transmodel<-lm(Peak.Demand**(lamdamax)~ WetBulb.min + RelHumi.avg + DryBulb.min + 
                 StPress.min + DewPoint.min + DewPoint.avg + DryBulb.avg + 
                 WetBulb.avg + WindSpeed.avg + RelHumi.max + StPress.max + 
                 DewPoint.max + WetBulb.max + DryBulb.max + RelHumi.min + 
                 WindSpeed.min + Population + PopDensity, data = train.df)
...
#Boxcox for mlr5
BC<-boxcox(Peak.Demand ~ WindSpeed.avg + RelHumi.avg + DewPoint.avg + DryBulb.avg + PopDensity + StPress.max, data=train.df ), data=train.df,lambda = seq(-4,4))
lamdamax<- BC$x[which.max(BC$y)]
lamdamax
...
x.train <- as.matrix(cbind(train.df[,c(2:13,15:18,20:22)]))
#
...
x.test<- as.matrix(cbind(test.df[,c(2:13,15:18,20:22)]))
...
#WetBulb.min, RelHumi.max, DewPoint.max, WindSpeed.avg, DryBulb.max, StPress.min
#RMSE is 173 while R-square is 0.1309
...
# RMSE insample is 166

#The important variables from Lasso are
#DryBulb.avg, WetBulb.min, DewPoint.max, RelHumi.avg, WindSpeed.avg, StPress.min, Area, PopDensity
...
folds <- 10
ri1$folds<- sample(seq(1:folds),size=nrow(ri1),replace = T)
...
for(i in 1:10){
  set.seed(9)
  fold<-which(ri1$folds==i)
  testData<-ri1[which(ri1$folds==i),]
  trainData<-ri1[-which(ri1$folds==i),]
  
  
  ordinary.model<-lm(Peak.Demand ~  WindSpeed.avg + RelHumi.avg + DewPoint.max + DryBulb.avg + WetBulb.min + PopDensity + StPress.min + AREA.sqm., data=trainData)
  # Out of Sample
  ...
  x.train.ridge <- as.matrix(cbind(trainData[,c(6,11,8,16,2,18,21,22)])) 
  y.test.ridge <- as.matrix(testData$Peak.Demand)
  x.test.ridge <- as.matrix(cbind(testData[,c(6,11,8,16,2,18,21,22)]))
  ...
  x.train.lasso <- as.matrix(cbind(trainData[,c(4,6,8,13,16,18,21,22)]))
  y.test.lasso<- as.matrix(testData$Peak.Demand)
  x.test.lasso <- as.matrix(cbind(testData[,c(4,6,8,13,16,18,21,22)]))
...
gam.1<-gam(Peak.Demand ~ WindSpeed.avg + RelHumi.avg + DewPoint.max + DryBulb.avg + WetBulb.min + PopDensity + StPress.min + AREA.sqm., data=trainData)

gam.step <- step.gam(gam.1,scope = list("DryBulb.avg" =~ 1+ DryBulb.avg+ s(DryBulb.avg, df = 2)+ s(DryBulb.avg, df = 3)+ s(DryBulb.avg, df = 5),         
                                        
                                        "WetBulb.min" =~ 1+ WetBulb.min+ s(WetBulb.min, df = 2)+ s(WetBulb.min, df = 3)+ s(WetBulb.min, df = 5), 
                                        
                                        "DewPoint.max" =~ 1+ DewPoint.max+ s(DewPoint.max, df = 2)+ s(DewPoint.max, df = 3)+ s(DewPoint.max, df = 5), 
...
for(i in 1:10){
  ...
  gam.2<-gam(Peak.Demand ~  s(DryBulb.avg, df = 5) + s(WetBulb.min, df = 5) + s(DewPoint.max, df = 5)
             + s(RelHumi.avg, df = 3) + s(WindSpeed.avg,df = 5) + 
               s(StPress.min, df = 5) + s(AREA.sqm., df = 3) +PopDensity, data = trainData)
