...
ri <- read.table("E:/Purdue University/Spring 2018/IE 590 - Predictive Modeling/Final Project/final Dataset/RI.csv", sep = ",", header = T, stringsAsFactors = F)

final.df <- ri[,c(-1,-2,-4,-25,-24, -23)]
...
for(i in 1:10)
{
  ...
  random.forest.model.fl<-randomForest(Peak.Demand ~ HOURLYDRYBULBTEMPC.avg + HOURLYWETBULBTEMPC.avg + HOURLYDewPointTempC.avg + HOURLYRelativeHumidity.avg + PopDensity,data=trainData,ntree=500,mtry = 4,importance=T)
  ...
  ################################ Model 3A-MARS(Unpruned) ################################
  mars.unpruned.model<- earth(Peak.Demand~., data=trainData,pmethod = 'none')
  ...
  ################################ Model 3B-MARS(Pruned) ################################
  mars.pruned.model<-earth(Peak.Demand~., data=trainData)
