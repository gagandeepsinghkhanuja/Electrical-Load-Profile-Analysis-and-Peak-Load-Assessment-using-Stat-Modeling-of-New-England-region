...
############################# Extracting and merging Hampshire Data ############################# 
df.parameters.hs <- read.csv("MA.csv",sep =',', header = TRUE)
county.hs<- filter(df.parameters.hs,COUNTY =="Hampshire")
county.hs

df.peak.hs <- read.csv("WCMASS-Peak-Demand.csv",sep =',', header = TRUE)
df.peak.hs$STATE <- "MA"
df.countyfourteen <- cbind(df.peak.hs)
df.countyfourteen$COUNTY <- "Hampshire"
df.countyfourteen

merged.data.hs <- merge(x=df.countyfourteen[,c(1,2,3,4)],y=county.hs[,c(5:23)],by.x="Date",by.y="DATE")
write.csv(merged.data.hs,"MA-WCMASS-Hampshire.csv",row.names = FALSE)




############################# Merging all the dataframes to one ############################# 
df.merge <- Reduce(function(x, y) merge(x, y, all=TRUE), list(merged.data.br,merged.data.py,merged.data.bs,merged.data.dk,merged.data.nk,merged.data.ex,merged.data.mx,merged.data.sf,merged.data.nk,merged.data.bk,merged.data.fi,merged.data.hp,merged.data.wc,merged.data.hs))
