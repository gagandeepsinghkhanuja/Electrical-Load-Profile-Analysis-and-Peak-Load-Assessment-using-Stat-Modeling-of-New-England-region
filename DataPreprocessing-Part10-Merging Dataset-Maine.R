...
df.parameters.fk <- read.csv("ME.csv",sep =',', header = TRUE)
county.fk<- filter(df.parameters.fk,COUNTY =="Franklin")
county.fk

df.peak.fk <- read.csv("ME-Peak-Demand.csv",sep =',', header = TRUE)
df.peak.fk$STATE <- "ME"
df.countytwelve <- cbind(df.peak.fk)
df.countytwelve$COUNTY <- "Franklin"
df.countytwelve

merged.data.fk <- merge(x=df.countytwelve[,c(1,2,3,4)],y=county.fk[,c(5:23)],by.x="Date",by.y="DATE")
write.csv(merged.data.fk,"ME-Franklin.csv",row.names = FALSE)


############################# Extracting and merging Sagadahoc Data ############################# 
df.parameters.sh <- read.csv("ME.csv",sep =',', header = TRUE)
county.sh<- filter(df.parameters.sh,COUNTY =="Sagadahoc")
county.sh

df.peak.sh <- read.csv("ME-Peak-Demand.csv",sep =',', header = TRUE)
df.peak.sh$STATE <- "ME"
df.countythirteen <- cbind(df.peak.sh)
df.countythirteen$COUNTY <- "Sagadahoc"
df.countythirteen

merged.data.sh <- merge(x=df.countythirteen[,c(1,2,3,4)],y=county.sh[,c(5:23)],by.x="Date",by.y="DATE")
write.csv(merged.data.sh,"ME-Sagadahoc.csv",row.names = FALSE)


############################# Extracting and merging Somerset Data ############################# 
df.parameters.ss <- read.csv("ME.csv",sep =',', header = TRUE)
county.ss<- filter(df.parameters.ss,COUNTY =="Somerset")
county.ss

df.peak.ss <- read.csv("ME-Peak-Demand.csv",sep =',', header = TRUE)
df.peak.ss$STATE <- "ME"
df.countyfourteen <- cbind(df.peak.ss)
df.countyfourteen$COUNTY <- "Somerset"
df.countyfourteen

merged.data.ss <- merge(x=df.countyfourteen[,c(1,2,3,4)],y=county.ss[,c(5:23)],by.x="Date",by.y="DATE")
write.csv(merged.data.ss,"ME-Somerset.csv",row.names = FALSE)


############################# Extracting and merging Waldo Data ############################# 
df.parameters.wo <- read.csv("ME.csv",sep =',', header = TRUE)
county.wo<- filter(df.parameters.wo,COUNTY =="Waldo")
county.wo

df.peak.wo <- read.csv("ME-Peak-Demand.csv",sep =',', header = TRUE)
df.peak.wo$STATE <- "ME"
df.countyfifteen <- cbind(df.peak.wo)
df.countyfifteen$COUNTY <- "Waldo"
df.countyfifteen

merged.data.wo <- merge(x=df.countyfifteen[,c(1,2,3,4)],y=county.wo[,c(5:23)],by.x="Date",by.y="DATE")
write.csv(merged.data.wo,"ME-Waldo.csv",row.names = FALSE)


############################# Extracting and merging Washington Data ############################# 
df.parameters.wg <- read.csv("ME.csv",sep =',', header = TRUE)
county.wg<- filter(df.parameters.wg,COUNTY =="Washington")
county.wg

df.peak.wg <- read.csv("ME-Peak-Demand.csv",sep =',', header = TRUE)
df.peak.wg$STATE <- "ME"
df.countysixteen <- cbind(df.peak.wg)
df.countysixteen$COUNTY <- "Washington"
df.countysixteen

merged.data.wg <- merge(x=df.countysixteen[,c(1,2,3,4)],y=county.wg[,c(5:23)],by.x="Date",by.y="DATE")
write.csv(merged.data.wg,"ME-Washington.csv",row.names = FALSE)

############################# Merging all the dataframes to one ############################# 
df.merge <- Reduce(function(x, y) merge(x, y, all=TRUE), list(merged.data.ag,merged.data.as,merged.data.cl,merged.data.hc,merged.data.ke,merged.data.kx,merged.data.lc,merged.data.ox,merged.data.pt,merged.data.pq,merged.data.yk,merged.data.fk,merged.data.sh,merged.data.ss,merged.data.wo,merged.data.wg))
