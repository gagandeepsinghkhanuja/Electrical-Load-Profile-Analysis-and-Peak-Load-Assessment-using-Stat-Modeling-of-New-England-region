...
vt1 <- read.table("F:/Purdue/Predictive Modelling/Project/FINAL DATA/vt.csv", sep = ",", header = T, stringsAsFactors = F)
vt1$Date <- as.Date(vt$Date)
vt1<-vt1[,c(3,5:23,25,26)]
...
ri1 <- read.table("F:/Purdue/Predictive Modelling/Project/FINAL DATA/ri.csv", sep = ",", header = T, stringsAsFactors = F)
ri1$Date <- as.Date(ri1$Date)
ri1<-ri[,c(3,5:23,25,26)]
...
par(mfrow=c(3,2))
for (i in 1:22) {
  hist(vt1[,i], xlab=names(vt1)[i], main=names(vt1)[i], col="blue", prob=T)
  lines(density(vt1[,i]), col="red", lwd=2) #adds density line
}
...
library(corrplot)
library(Hmisc)
r <- cor(vt[,c(3,5:23,25,26)])
res<- rcorr(as.matrix(vt[,c(3,5:23,25,26)]))
...
#we keep DryBulb.avg, WetBulb.avg, Dewpoint.min, StPress.max
#and remove date,state,county and year

vt2<-subset(vt1, select = c(-2,-3,-5,-6,-8,-10,-18,-19,-20))
...
plot1<-ggplot(vt, aes(x=Peak.Demand))+ 
...
scplot1<-ggplot(vt, aes(x=DryBulb.avg, y=Peak.Demand)) + 
...
mlr1<-lm(Peak.Demand~., data = vt2)
summary(mlr1)
#As seen from the ANOVA we drop minimum and average Relative Humidity
#As seen from the ANOVA we drop maximum and minimum Wind Speed
#We drop Area too

vt3<-subset(vt2, select = c(-6,-7,-8,-9,-12))
mlr2<-lm(Peak.Demand~.,data = vt3)
summary(mlr2)
alias(mlr2)
#Very low R-square value 0.2356 


#Training and Testing------------------------------------------------------------------------------
#Splitting the data into training and testing sets
sample_data<-sample(length(vt1[,1]),size=0.85*(length(vt1[,1])),replace=FALSE,set.seed(9))
train.df<-vt1[sample_data,]
test.df<-vt1[-sample_data,]
...
mlr4<-lm(Peak.Demand ~DryBulb.avg + DewPoint.avg + RelHumi.avg + 
           WetBulb.avg + DewPoint.min + RelHumi.max + WindSpeed.avg + 
           DruBulb.max + RelHumi.min + WetBulb.max + DewPoint.max + 
           WindSpeed.max + WetBulb.min + DryBulb.min + WindSpeed.min + 
           StPress.max + StPress.avg + Population + PopDensity + AREA.sqm., data = train.df)
summary(mlr4)
#R-square improves to 0.3259

#Taking only those variables with high value co-efficients seen in Stepwise
mlr5<-lm(Peak.Demand ~ WindSpeed.avg + RelHumi.avg + DewPoint.avg + DryBulb.avg + PopDensity + StPress.max, data=train.df )
summary(mlr5)
...
rmse.mlr
#RMSE of 63 achieved. Therefore, further investigation required
...
library('MASS')
#Boxcox for mlr4
BC<-boxcox(Peak.Demand ~DryBulb.avg + DewPoint.avg + RelHumi.avg + 
             WetBulb.avg + DewPoint.min + RelHumi.max + WindSpeed.avg + 
             DruBulb.max + RelHumi.min + WetBulb.max + DewPoint.max + 
             WindSpeed.max + WetBulb.min + DryBulb.min + WindSpeed.min + 
             StPress.max + StPress.avg + Population + PopDensity + AREA.sqm., data = train.df,lambda = seq(-4,4))
lamdamax<- BC$x[which.max(BC$y)]
lamdamax
transmodel<-lm(Peak.Demand**(lamdamax)~ DryBulb.avg + DewPoint.avg + RelHumi.avg + 
                 WetBulb.avg + DewPoint.min + RelHumi.max + WindSpeed.avg + 
                 DruBulb.max + RelHumi.min + WetBulb.max + DewPoint.max + 
                 WindSpeed.max + WetBulb.min + DryBulb.min + WindSpeed.min + 
                 StPress.max + StPress.avg + Population + PopDensity + AREA.sqm., data = train.df)
...
#Boxcox for mlr5
BC<-boxcox(Peak.Demand ~  WindSpeed.avg + RelHumi.avg + DewPoint.avg + DryBulb.avg + PopDensity + StPress.max, data=train.df,lambda = seq(-4,4))
lamdamax<- BC$x[which.max(BC$y)]
lamdamax
transmodel<-lm(Peak.Demand**(lamdamax)~ WindSpeed.avg + RelHumi.avg + DewPoint.avg + DryBulb.avg + PopDensity + StPress.max, data=train.df)
summary(transmodel)
...
x.train <- as.matrix(cbind(train.df[,c(2:17,19:22)]))
#
...
x.test<- as.matrix(cbind(test.df[,c(2:17,19:22)]))
...
#WetBulb.avg, RelHumi.max, DewPoint.max, WindSpeed.avg, DryBulb.avg, StPress.max
#RMSE is 66 while R-square is 0.2406
...
#DryBulb.avg, WetBulb.avg, DewPoint.avg, RelHumi.avg, WindSpeed.avg, StPress.avg, Area, PopDensity
...
folds <- 10
vt1$folds<- sample(seq(1:folds),size=nrow(vt1),replace = T)
...
for(i in 1:10){
  set.seed(9)
  fold<-which(vt1$folds==i)
  testData<-vt1[which(vt1$folds==i),]
  trainData<-vt1[-which(vt1$folds==i),]
  
  
  ordinary.model<-lm(Peak.Demand ~  WindSpeed.avg + RelHumi.avg + DewPoint.avg + DryBulb.avg + WetBulb.avg + PopDensity + StPress.avg + AREA.sqm., data=trainData)
  # Out of Sample
  ...
  x.train.ridge <- as.matrix(cbind(trainData[,c(7,11,8,16,4,17,21,22)])) 
  y.test.ridge <- as.matrix(testData$Peak.Demand)
  x.test.ridge <- as.matrix(cbind(testData[,c(7,11,8,16,4,17,21,22)]))
  ...
  x.train.lasso <- as.matrix(cbind(trainData[,c(4,7,10,13,16,19,21,22)]))
  y.test.lasso<- as.matrix(testData$Peak.Demand)
  x.test.lasso <- as.matrix(cbind(testData[,c(4,7,10,13,16,19,21,22)]))
...
gam.1<-gam(Peak.Demand ~ WindSpeed.avg + RelHumi.avg + DewPoint.avg + DryBulb.avg + WetBulb.avg + PopDensity + StPress.avg + AREA.sqm., data=trainData)

gam.step <- step.gam(gam.1,scope = list("DryBulb.avg" =~ 1+ DryBulb.avg+ s(DryBulb.avg, df = 2)+ s(DryBulb.avg, df = 3)+ s(DryBulb.avg, df = 5),         
                                        
                                        "WetBulb.avg" =~ 1+ WetBulb.avg+ s(WetBulb.avg, df = 2)+ s(WetBulb.avg, df = 3)+ s(WetBulb.avg, df = 5), 
                                        ...
                                        "WindSpeed.avg" =~ 1+ WindSpeed.avg+ s(WindSpeed.avg, df = 2)+ s(WindSpeed.avg, df = 3)+ s(WindSpeed.avg, df = 5),
...
for(i in 1:10){
  ...
  gam.2<-gam(Peak.Demand ~   s(DryBulb.avg, df = 5) + s(DewPoint.avg, df = 5) + s(RelHumi.avg, df = 5)
             + s(WindSpeed.avg, df = 5) + s(StPress.avg,df = 5) + s(AREA.sqm., df = 5) 
