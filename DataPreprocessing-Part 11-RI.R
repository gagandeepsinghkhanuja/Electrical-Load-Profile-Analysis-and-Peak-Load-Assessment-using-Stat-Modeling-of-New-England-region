...
setwd("E:/Purdue University/Spring 2018/IE 590 - Predictive Modeling/Final Project/Data Preprocessing - Part11(Population and parameters)/RI")


############################ Mapping the RI - Bristol Population with Dates ############################ 
...
ri.bristol.merged <- rbind(ri.population.2011,ri.population.2012,ri.population.2013,ri.population.2014,ri.population.2015,ri.population.2016,ri.population.2017)
ri.bristol.merged
write.csv(ri.bristol.merged,"RI-Bristol-Population.csv", row.names = FALSE)


# Reading the csv that was generated in the previous step.
ri.bristol.population <- read.csv("RI-Bristol-Population.csv",header = TRUE)


# Reading the csv that contains all the parameters
ri.bristol <- read.csv("RI-Bristol.csv",header = TRUE)
ri.bristol


# Merge the Dataset based on Dates.
merged.data.br <- merge(x=ri.bristol[,c(1:22)],y=ri.bristol.population[,c(1,4)],by.x="Date",by.y="Date")
write.csv(merged.data.br,"RI-Bristol-Final.csv", row.names = FALSE)
...
ri.population.2011 <- data.frame(Date = seq(as.Date("2011-01-01"),as.Date("2011-12-31"), by = "days"),
                                 State = "RI", 
                                 County = "Washington",
                                 Population = as.numeric(gsub(",", "", ri.population$POP_ESTIMATE_2011[5]))
)

ri.population.2012 <- data.frame(Date = seq(as.Date("2012-01-01"),as.Date("2012-12-31"), by = "days"),
                                 State = "RI", 
                                 County = "Washington",
                                 Population = as.numeric(gsub(",", "", ri.population$POP_ESTIMATE_2012[5]))
)

ri.population.2013 <- data.frame(Date = seq(as.Date("2013-01-01"),as.Date("2013-12-31"), by = "days"),
                                 State = "RI", 
                                 County = "Washington",
                                 Population = as.numeric(gsub(",", "", ri.population$POP_ESTIMATE_2013[5]))
)

ri.population.2014 <- data.frame(Date = seq(as.Date("2014-01-01"),as.Date("2014-12-31"), by = "days"),
                                 State = "RI", 
                                 County = "Washington",
                                 Population = as.numeric(gsub(",", "", ri.population$POP_ESTIMATE_2014[5]))
)

ri.population.2015 <- data.frame(Date = seq(as.Date("2015-01-01"),as.Date("2015-12-31"), by = "days"),
                                 State = "RI", 
                                 County = "Washington",
                                 Population = as.numeric(gsub(",", "", ri.population$POP_ESTIMATE_2015[5]))
)

ri.population.2016 <- data.frame(Date = seq(as.Date("2016-01-01"),as.Date("2016-12-31"), by = "days"),
                                 State = "RI", 
                                 County = "Washington",
                                 Population = as.numeric(gsub(",", "", ri.population$POP_ESTIMATE_2016[5]))
)
...
df.merge <- Reduce(function(x, y) merge(x, y, all=TRUE), list(merged.data.br,merged.data.kn,merged.data.np,merged.data.pr,merged.data.ws))
df.merge.sorted <- df.merge[order(df.merge$COUNTY),]
write.csv(df.merge.sorted,"RI.csv",row.names = FALSE)
