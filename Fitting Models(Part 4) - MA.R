...
ma <- read.table("/home/dmaiasil/Desktop/MA.csv", sep=",", stringsAsFactors = F, header = T)
...
#install.packages("ModelMetrics", Sys.getenv("/home/dmaiasil/Rlib_new"))
#install.packages("caret", Sys.getenv("/home/dmaiasil/Rlib_new"))

#install.packages("e1071", Sys.getenv("/home/dmaiasil/Rlib_new"))

#install.packages("LncFinder", Sys.getenv("/home/dmaiasil/Rlib_new"))
...
ma <- subset(ma, select = c(-1,-2,-4,-25,-24, -23))

#####SVM Parameters####
set.seed(9)

folds <- cut(seq(1,nrow(ma)),breaks=10,labels=FALSE)
...
inTrain <- createDataPartition(y = ma$Peak.Demand,
                               p = .85,            
                               list = FALSE)
train <- ma[inTrain,]
test <- ma[-inTrain,]
...
save(list=ls(all=T),file='MA_SVM.RData')
