...
df.parameters.bo <- read.csv("RI.csv",sep =',', header = TRUE)
county.bo<- filter(df.parameters.bo,COUNTY =="Bristol")
county.bo

df.peak.bo <- read.csv("RI-Peak-Demand.csv",sep =',', header = TRUE)
df.peak.bo$STATE <- "RI"
df.countyfive <- cbind(df.peak.bo)
df.countyfive$COUNTY <- "Bristol"
df.countyfive

merged.data.bo <- merge(x=df.countyfive[,c(1,2,3,4)],y=county.bo[,c(5:23)],by.x="Date",by.y="DATE")
write.csv(merged.data.bo,"RI-Bristol.csv",row.names = FALSE)



############################# Merging all the dataframes to one ############################# 
df.merge <- Reduce(function(x, y) merge(x, y, all=TRUE), list(merged.data.kn,merged.data.np,merged.data.pr,merged.data.ws,merged.data.bo))
