...
setwd("E:/Purdue University/Spring 2018/IE 590 - Predictive Modeling/Final Project/Data Preprocessing - Part11(Population and parameters)/NH")
...
############################ Mapping the NH - Carroll Population with Dates ############################ 
# Read the CSV file and import it to a dataframe.
nh.population <- read.csv("NH-PopulationCounty.csv", header = TRUE)
nh.population
...
nh.carroll.merged <- rbind(nh.population.2011,nh.population.2012,nh.population.2013,nh.population.2014,nh.population.2015,nh.population.2016,nh.population.2017)
nh.carroll.merged
write.csv(nh.carroll.merged,"NH-Carroll-Population.csv", row.names = FALSE)


# Reading the csv that was generated in the previous step.
nh.carroll.population <- read.csv("NH-Carroll-Population.csv",header = TRUE)


# Reading the csv that contains all the parameters
nh.carroll <- read.csv("NH-Carroll.csv",header = TRUE)
nh.carroll


# Merge the Dataset based on Dates.
merged.data.cr <- merge(x=nh.carroll[,c(1:22)],y=nh.carroll.population[,c(1,4)],by.x="Date",by.y="Date")
write.csv(merged.data.cr,"NH-Carroll-Final.csv", row.names = FALSE)
...
nh.population.2011 <- data.frame(Date = seq(as.Date("2011-01-01"),as.Date("2011-12-31"), by = "days"),
                                 State = "NH", 
                                 County = "Strafford",
                                 Population = as.numeric(gsub(",", "", nh.population$POP_ESTIMATE_2011[9]))
)

nh.population.2012 <- data.frame(Date = seq(as.Date("2012-01-01"),as.Date("2012-12-31"), by = "days"),
                                 State = "NH", 
                                 County = "Strafford",
                                 Population = as.numeric(gsub(",", "", nh.population$POP_ESTIMATE_2012[9]))
)

nh.population.2013 <- data.frame(Date = seq(as.Date("2013-01-01"),as.Date("2013-12-31"), by = "days"),
                                 State = "NH", 
                                 County = "Strafford",
                                 Population = as.numeric(gsub(",", "", nh.population$POP_ESTIMATE_2013[9]))
)

nh.population.2014 <- data.frame(Date = seq(as.Date("2014-01-01"),as.Date("2014-12-31"), by = "days"),
                                 State = "NH", 
                                 County = "Strafford",
                                 Population = as.numeric(gsub(",", "", nh.population$POP_ESTIMATE_2014[9]))
)

nh.population.2015 <- data.frame(Date = seq(as.Date("2015-01-01"),as.Date("2015-12-31"), by = "days"),
                                 State = "NH", 
                                 County = "Strafford",
                                 Population = as.numeric(gsub(",", "", nh.population$POP_ESTIMATE_2015[9]))
)

nh.population.2016 <- data.frame(Date = seq(as.Date("2016-01-01"),as.Date("2016-12-31"), by = "days"),
                                 State = "NH", 
                                 County = "Strafford",
                                 Population = as.numeric(gsub(",", "", nh.population$POP_ESTIMATE_2016[9]))
)
...
############################ Mapping the NH - Sullivan Population with Dates ############################ 
# Read the CSV file and import it to a dataframe.
nh.population <- read.csv("NH-PopulationCounty.csv", header = TRUE)
nh.population
...
nh.population.2011 <- data.frame(Date = seq(as.Date("2011-01-01"),as.Date("2011-12-31"), by = "days"),
                                 State = "NH", 
                                 County = "Sullivan",
                                 Population = as.numeric(gsub(",", "", nh.population$POP_ESTIMATE_2011[10]))
)

nh.population.2012 <- data.frame(Date = seq(as.Date("2012-01-01"),as.Date("2012-12-31"), by = "days"),
                                 State = "NH", 
                                 County = "Sullivan",
                                 Population = as.numeric(gsub(",", "", nh.population$POP_ESTIMATE_2012[10]))
)

nh.population.2013 <- data.frame(Date = seq(as.Date("2013-01-01"),as.Date("2013-12-31"), by = "days"),
                                 State = "NH", 
                                 County = "Sullivan",
                                 Population = as.numeric(gsub(",", "", nh.population$POP_ESTIMATE_2013[10]))
)

nh.population.2014 <- data.frame(Date = seq(as.Date("2014-01-01"),as.Date("2014-12-31"), by = "days"),
                                 State = "NH", 
                                 County = "Sullivan",
                                 Population = as.numeric(gsub(",", "", nh.population$POP_ESTIMATE_2014[10]))
)

nh.population.2015 <- data.frame(Date = seq(as.Date("2015-01-01"),as.Date("2015-12-31"), by = "days"),
                                 State = "NH", 
                                 County = "Sullivan",
                                 Population = as.numeric(gsub(",", "", nh.population$POP_ESTIMATE_2015[10]))
)

nh.population.2016 <- data.frame(Date = seq(as.Date("2016-01-01"),as.Date("2016-12-31"), by = "days"),
                                 State = "NH", 
                                 County = "Sullivan",
                                 Population = as.numeric(gsub(",", "", nh.population$POP_ESTIMATE_2016[10]))
)
...
nh.sullivan.merged <- rbind(nh.population.2011,nh.population.2012,nh.population.2013,nh.population.2014,nh.population.2015,nh.population.2016,nh.population.2017)
nh.sullivan.merged
write.csv(nh.sullivan.merged,"NH-Sullivan-Population.csv", row.names = FALSE)


# Reading the csv that was generated in the previous step.
nh.sullivan.population <- read.csv("NH-Sullivan-Population.csv",header = TRUE)


# Reading the csv that contains all the parameters
nh.sullivan <- read.csv("NH-Sullivan.csv",header = TRUE)
nh.sullivan


# Merge the Dataset based on Dates.
merged.data.sl <- merge(x=nh.sullivan[,c(1:22)],y=nh.sullivan.population[,c(1,4)],by.x="Date",by.y="Date")
write.csv(merged.data.sl,"NH-Sullivan-Final.csv", row.names = FALSE)
...
df.merge <- Reduce(function(x, y) merge(x, y, all=TRUE), list(merged.data.bl,merged.data.cr,merged.data.ch,merged.data.co,merged.data.gf,merged.data.ho,merged.data.mk,merged.data.ro,merged.data.sf,merged.data.sl))
df.merge.sorted <- df.merge[order(df.merge$COUNTY),]
write.csv(df.merge.sorted,"NH.csv",row.names = FALSE)
